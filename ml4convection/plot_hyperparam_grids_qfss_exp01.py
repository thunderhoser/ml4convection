"""Plots evaluation metrics vs. hyperparameters for qFSS Experiment 1."""
...
USE_FANCY_QUANTILES_HELP_STRING = (
    'Boolean flag.  If 1, will use fancy quantile-based method to compute '
    'standard deviation of predictive distribution.  If False, will treat each '
    'quantile-based estimate as a Monte Carlo estimate.'
)
...
def _print_ranking_all_scores(
    ...
    :param aupd_matrix: W-by-Q numpy array with AUPD (area under performance
        diagram).
    ...
    for k in range(len(i_sort_indices)):
        ...
        print((
            '{0:d}th-best model ... FSS weight = {1:.1f} ... '
            ...
            k + 1, FSS_WEIGHTS[i], QUANTILE_LEVEL_COUNTS[j],
...
def _run(experiment_dir_name, matching_distance_px,
    ...
    for i in range(num_fss_weights):
        for j in range(num_quantile_level_counts):
            ...
            this_score_file_name = (
                '{0:s}/fss-weight={1:04.1f}_num-quantile-levels={2:03d}/'
                'validation_with_uq/partial_grids/evaluation_{3:s}_stdev/'
                'spread_vs_skill_matching-distance-px=0.000000.nc'
            ).format(
                experiment_dir_name, FSS_WEIGHTS[i], QUANTILE_LEVEL_COUNTS[j],
                'fancy' if use_fancy_quantile_method_for_stdev else 'simple'
            )
