"""Makes U-net templates for CRPS Experiment 1."""
...
OUTPUT_DIR_NAME = (
    '{0:s}/ml4convection_models/crps_experiment01/templates'
...
def _run():
    ...
    for i in range(num_neigh_sizes):
        for j in range(num_crps_weights):
            ...
            option_dict[u_net_architecture.SKIP_DROPOUT_RATES_KEY][0] = (
                numpy.full(2, TOP_LEVEL_SKIP_DROPOUT_RATE)
            )

            loss_function_name = neural_net.metric_params_to_name(
                score_name=neural_net.FSS_PLUS_CRPS_NAME,
            ...
            model_object = u_net_architecture.create_crps_model(
                ...
                mask_matrix=partial_mask_matrix, num_estimates=50
            )

            model_file_name = '{0:s}/{1:s}/model.h5'.format(
                OUTPUT_DIR_NAME, loss_function_name
            )
