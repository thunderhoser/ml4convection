"""Ranks each UQ metric over all models for UQ Experiment 2."""
...
PENULTIMATE_LAYER_DROPOUT_RATES = numpy.linspace(0, 0.8, num=5)
OUTPUT_LAYER_DROPOUT_RATES = numpy.linspace(0, 0.8, num=5)
...
def _run(experiment_dir_name, matching_distance_px, output_dir_name):
    ...
    for k in range(num_output_layer_rates):
        ...
        axes_object.set_title(
            'SS quality with output-layer dropout rate = {0:.3f}'.format(
                OUTPUT_LAYER_DROPOUT_RATES[k]
            )
        )
        figure_file_name = (
        ...
        title_string = (
            'Monotonicity fraction with output-layer dropout rate = {0:.3f}'
        ).format(
            OUTPUT_LAYER_DROPOUT_RATES[k]
        )
