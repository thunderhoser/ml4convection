...
def _run(experiment_dir_name, matching_distance_px, output_dir_name):
    ...
    for i in range(num_half_window_sizes):
        ...
        axes_object.set_title('AUPD for {0:d}-by-{0:d} loss'.format(
        ...
        axes_object.set_title('Max CSI for {0:d}-by-{0:d} loss'.format(
            2 * HALF_WINDOW_SIZES_PX[i] + 1
        ))
        ...
        axes_object.set_title('FSS for {0:d}-by-{0:d} loss'.format(
            2 * HALF_WINDOW_SIZES_PX[i] + 1
        ))
        ...
        axes_object.set_title('BSS for {0:d}-by-{0:d} loss'.format(
            2 * HALF_WINDOW_SIZES_PX[i] + 1
        ))
