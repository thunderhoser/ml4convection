"""Plots scores on hyperparameter grid for Experiment 13c."""
...
U_NET_TYPE_STRINGS = ['vanilla', 'temporal', 'plusplus']
U_NET_TYPE_STRINGS_FANCY = ['vanilla U-nets', 'temporal U-nets', 'U-net++']
...
LAG_TIME_STRINGS_MINUTES = [
    '0,10,20', '0,10,20,30,40', '0,10,20,30,40,50,60',
...
SELECTED_MARKER_INDICES = numpy.array([0, 2, 3, 4], dtype=int)
...
def _print_ranking_one_score(score_matrix, score_name):
    ...
    T = number of U-net types
    B = number of batch sizes
    W = number of L_2 weights
    L = number of lag-time counts

    :param score_matrix: T-by-B-by-W-by-L numpy array of scores.
    ...
    (
        u_net_type_indices, batch_size_indices,
        l2_weight_indices, lag_time_indices
    ) = numpy.unravel_index(sort_indices_1d, score_matrix.shape)

    for rank in range(len(u_net_type_indices)):
        i = u_net_type_indices[rank]
        j = batch_size_indices[rank]
        k = l2_weight_indices[rank]
        m = lag_time_indices[rank]

        print((
            '{0:d}th-highest {1:s} = {2:.4g} ... U-net type = {3:s} ... '
            'batch size = {4:d} ... L_2 weight = 10^{5:.1f} ... '
            'lag times in minutes = {6:s}'
        ).format(
            rank + 1, score_name, score_matrix[i, j, k, m],
            U_NET_TYPE_STRINGS[i], BATCH_SIZES[j],
            numpy.log10(L2_WEIGHTS[k]), LAG_TIME_STRINGS_MINUTES[m]
        ))


def _add_markers(
        figure_object, axes_object, u_net_type_index, batch_size_index,
        best_marker_indices):
    ...
    :param axes_object: Axes handle (instance of
        `matplotlib.axes._subplots.AxesSubplot`).
    :param u_net_type_index: Array index for U-net type.
    :param batch_size_index: Array index for batch size.
    :param best_marker_indices: length-4 numpy array of array indices for best
        model.
    """

    if (
            best_marker_indices[0] == u_net_type_index and
            best_marker_indices[1] == batch_size_index
    ):
        figure_width_px = (
        ...
        axes_object.plot(
            best_marker_indices[3], best_marker_indices[2],
    ...
    if (
            SELECTED_MARKER_INDICES[0] == u_net_type_index and
            SELECTED_MARKER_INDICES[1] == batch_size_index
    ):
        figure_width_px = (
            figure_object.get_size_inches()[0] * figure_object.dpi
        )
        ...
        axes_object.plot(
            SELECTED_MARKER_INDICES[3], SELECTED_MARKER_INDICES[2],
...
def _run(experiment_dir_name, matching_distance_px, output_dir_name):
    ...
    num_u_net_types = len(U_NET_TYPE_STRINGS)
    num_batch_sizes = len(BATCH_SIZES)
    num_l2_weights = len(L2_WEIGHTS)
    num_lag_time_sets = len(LAG_TIME_STRINGS_SEC)

    dimensions = (
        num_u_net_types, num_batch_sizes, num_l2_weights, num_lag_time_sets
    )
    ...
    for i in range(num_u_net_types):
        for j in range(num_batch_sizes):
            for k in range(num_l2_weights):
                for m in range(num_lag_time_sets):
                    this_subdir_name = (
                        '' if U_NET_TYPE_STRINGS[i] == 'temporal'
                        else U_NET_TYPE_STRINGS[i]
                    )

                    this_score_file_name = (
                        '{0:s}/{1:s}/batch-size={2:02d}_l2-weight={3:.10f}_'
                        'lag-times-sec={4:s}/validation/partial_grids/'
                        'evaluation/matching_distance_px={5:.6f}/'
                        'advanced_scores_gridded=0.p'
                    ).format(
                        experiment_dir_name, this_subdir_name,
                        BATCH_SIZES[j], L2_WEIGHTS[k], LAG_TIME_STRINGS_SEC[m],
                    ...
                    t = evaluation.read_advanced_score_file(
                        this_score_file_name
                    )
                    ...
                    max_csi_matrix[i, j, k, m] = numpy.nanmax(
                        t[evaluation.CSI_KEY].values
                    )
                    fss_matrix[i, j, k, m] = t[evaluation.FSS_KEY].values[0]
                    bss_matrix[i, j, k, m] = (
    ...
    _print_ranking_one_score(score_matrix=max_csi_matrix, score_name='CSI')
    print(SEPARATOR_STRING)
    ...
    for i in range(num_u_net_types):
        for j in range(num_batch_sizes):
            ...
            _add_markers(
                ...
                u_net_type_index=i, batch_size_index=j,
            ...
            axes_object.set_title(
                'AUPD for {0:s} with batch size = {1:d}'.format(
                    U_NET_TYPE_STRINGS_FANCY[i], BATCH_SIZES[j]
                )
            )
            figure_file_name = '{0:s}/aupd_{1:s}_batch-size={2:02d}.jpg'.format(
                output_dir_name, U_NET_TYPE_STRINGS[i], BATCH_SIZES[j]
            )
            ...
            # Plot max CSI.
            figure_object, axes_object = _plot_scores_2d(
                score_matrix=max_csi_matrix[i, j, ...],
            ...
            axes_object.set_title(
                'Maximum CSI for {0:s} with batch size = {1:d}'.format(
                    U_NET_TYPE_STRINGS_FANCY[i], BATCH_SIZES[j]
                )
            )
            figure_file_name = '{0:s}/csi_{1:s}_batch-size={2:02d}.jpg'.format(
                output_dir_name, U_NET_TYPE_STRINGS[i], BATCH_SIZES[j]
            )
            ...
            axes_object.set_title(
                'FSS for {0:s} with batch size = {1:d}'.format(
                    U_NET_TYPE_STRINGS_FANCY[i], BATCH_SIZES[j]
                )
            )
            figure_file_name = '{0:s}/fss_{1:s}_batch-size={2:02d}.jpg'.format(
                output_dir_name, U_NET_TYPE_STRINGS[i], BATCH_SIZES[j]
            )
            ...
            axes_object.set_title(
                'BSS for {0:s} with batch size = {1:d}'.format(
                    U_NET_TYPE_STRINGS_FANCY[i], BATCH_SIZES[j]
                )
            )
            figure_file_name = '{0:s}/bss_{1:s}_batch-size={2:02d}.jpg'.format(
                output_dir_name, U_NET_TYPE_STRINGS[i], BATCH_SIZES[j]
            )
