"""Plots scores on hyperparameter grid for Experiment 13."""
...
BATCH_SIZES = numpy.array(
    [96, 192, 288, 384, 480, 192, 288, 384, 480], dtype=int
)
TRAINING_BATCH_COUNTS = numpy.array(
    [40, 40, 40, 40, 40, 20, 13, 10, 8], dtype=int
)
...
def _print_ranking_one_score(score_matrix, score_name):
    ...
    B = number of batch configs
    W = number of L_2 weights

    :param score_matrix: B-by-W numpy array of scores.
    ...
    for m in range(len(i_sort_indices)):
        ...
        print((
            ...
            'num training batches per epoch = {4:d} ... L_2 weight = 10^{5:.1f}'
        ).format(
            m + 1, score_name, score_matrix[i, j],
            BATCH_SIZES[i], TRAINING_BATCH_COUNTS[i], numpy.log10(L2_WEIGHTS[j])
        ))
...
def _run(experiment_dir_name, matching_distance_px, output_dir_name):
    ...
    num_batch_configs = len(BATCH_SIZES)
    num_l2_weights = len(L2_WEIGHTS)
    dimensions = (num_batch_configs, num_l2_weights)
    ...
    y_tick_labels = [
        '{0:d}, {1:d}'.format(s, c)
        for s, c in zip(BATCH_SIZES, TRAINING_BATCH_COUNTS)
    ]
    x_tick_labels = [
    ...
    y_axis_label = 'Examples per batch, batches per epoch'
    x_axis_label = r'L$_{2}$ weight'

    for i in range(num_batch_configs):
        for j in range(num_l2_weights):
            this_score_file_name = (
                '{0:s}/batch-size={1:03d}_training-batch-count={2:02d}'
                '_l2-weight={3:.10f}/validation/partial_grids/evaluation/'
                'matching_distance_px={4:.6f}/'
                'advanced_scores_gridded=0.p'
            ).format(
                experiment_dir_name, BATCH_SIZES[i], TRAINING_BATCH_COUNTS[i],
                L2_WEIGHTS[j], matching_distance_px
            )
